...
export const useGameLogic = (options) => {
    ...
    useEffect(() => {
        setGameState(prevState => (Object.assign(Object.assign({}, prevState), { timeLeft: timeLimit })));
    }, [timeLimit]);
    // Update board when game number changes
    useEffect(() => {
        setGameState(prevState => (Object.assign(Object.assign({}, prevState), { board: gameNumber ? createBoardWithBlocks(gameNumber, gameEndsWith2SpecificPattern(gameNumber) || isMultipleOf50Match2(gameNumber, currentMatch), currentMatch) : createEmptyBoard(), currentPlayer: startingPlayer, isGameActive: true, winner: 0, timeLeft: timeLimit, humanMoveCount: 0, pieceAges: initializePieceAges(), player1MoveCount: 0, player2MoveCount: 0, mudZones: gameNumber ? generateMudZones(gameNumber) : [], stuckPieces: {}, isBlindPlay: gameNumber ? (gameEndsWith2SpecificPattern(gameNumber) || isMultipleOf50Match2(gameNumber, currentMatch)) : false, totalMoveCount: 0, blockShiftMoveCount: 0, blindPlayTriggerMove: 0, winningPieces: [] })));
    ...
    const handleCellClick = useCallback((row, col) => {
        ...
        if (isInMudZone(row, col, gameState.mudZones)) {
            ...
            finalStuckPieces = Object.assign(Object.assign({}, updatedStuckPieces), { [pieceKey]: 1 });
        }
        ...
        setGameState(prevState => (Object.assign(Object.assign({}, prevState), { board: updatedBoard, currentPlayer: prevState.currentPlayer === 1 ? 2 : 1, winner, isGameActive: newGameActive, timeLeft: newGameActive ? timeLimit : prevState.timeLeft, humanMoveCount: newHumanMoveCount, pieceAges: updatedPieceAges, player1MoveCount: newPlayer1MoveCount, player2MoveCount: newPlayer2MoveCount, stuckPieces: finalStuckPieces, totalMoveCount: newTotalMoveCount, blockShiftMoveCount: newBlockShiftMoveCount, blindPlayTriggerMove: newBlindPlayTriggerMove, isBlindPlay: shouldBeBlindPlay, winningPieces: winner > 0 ? winningPieces : [] })));
    ...
    // Reset game
    const resetGame = useCallback((newStartingPlayer) => {
        setGameState({
            board: gameNumber ? createBoardWithBlocks(gameNumber, gameEndsWith2SpecificPattern(gameNumber) || isMultipleOf50Match2(gameNumber, currentMatch), currentMatch) : createEmptyBoard(),
    ...
    const updateGameState = useCallback((newState) => {
        setGameState(prevState => (Object.assign(Object.assign({}, prevState), newState)));
    }, []);
    // Timer effect
    useEffect(() => {
        ...
        timerRef.current = window.setInterval(() => {
            setGameState(prevState => {
                if (prevState.timeLeft <= 1) {
                    ...
                    return Object.assign(Object.assign({}, prevState), { timeLeft: 0, isGameActive: false, winner: prevState.currentPlayer === 1 ? 2 : 1, winningPieces: [] // No winning pieces for timeout
                     });
                }
                return Object.assign(Object.assign({}, prevState), { timeLeft: prevState.timeLeft - 1 });
            });
        }, 1000);
        return () => {
            if (timerRef.current) {
