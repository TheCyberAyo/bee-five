...
export const removeTwoBlockedCells = (board) => {
...
export const isMultipleOf7Between500And1000 = (gameNumber) => {
...
export const isMultipleOf4From1000 = (gameNumber) => {
...
export const getAdventureStartingPlayer = (gameNumber) => {
...
export const isMultipleOf50Match2 = (gameNumber, currentMatch) => {
...
export const isMultipleOf50Match3 = (gameNumber, currentMatch) => {
...
export const isMultipleOf10Match1From60 = (gameNumber, currentMatch) => {
...
export const isMultipleOf10Match1From110 = (gameNumber, currentMatch) => {
...
export const isMultipleOf10Match1From810 = (gameNumber, currentMatch) => {
...
export const isMultipleOf10Match2From30 = (gameNumber, currentMatch) => {
...
export const isMultipleOf10Match2From1200 = (gameNumber, currentMatch) => {
...
export const gameEndsWith1After200 = (gameNumber) => {
...
export const isFirstGameOfFiveMatchSeries = (gameNumber) => {
...
export const gameEndsWith1InSpecifiedRanges = (gameNumber) => {
    if (gameNumber % 10 !== 1) {
        return false;
    }
    // Range 1: 500-700
...
export const getProgressiveBlockRules = (gameNumber) => {
...
export const addProgressiveBlocks = (board, blocksToAdd) => {
    const newBoard = board.map(row => [...row]);
...
export const findStrategicBlockPosition = (board) => {
    ...
    for (const pos of emptyPositions) {
        ...
        strategicPositions.push(Object.assign(Object.assign({}, pos), { priority }));
    }
...
const checkBlockingValue = (board, row, col, player) => {
    let value = 0;
    const directions = [
        [0, 1], // horizontal
        [1, 0], // vertical
...
const checkProximityValue = (board, row, col, player) => {
    let value = 0;
...
export const addStrategicBlock = (board) => {
    const newBoard = board.map(row => [...row]);
...
export const moveRandomBlockToStrategicPosition = (board) => {
    const newBoard = board.map(row => [...row]);
    const blockedPositions = [];
    // Find all blocked cells
    for (let row = 0; row < 10; row++) {
...
export const shiftAllBlocks = (board) => {
    const newBoard = board.map(row => [...row]);
    const blockedPositions = [];
    // Find all blocked cells
    for (let row = 0; row < 10; row++) {
...
export const removeOldPieces = (board, pieceAges, turnsToLive) => {
    const newBoard = board.map(row => [...row]);
...
export const ageAllPieces = (board, pieceAges) => {
    const newPieceAges = pieceAges.map(row => [...row]);
...
export const removeOldestPiecesOfPlayer = (board, pieceAges, player, piecesToRemove = 1) => {
    const newBoard = board.map(row => [...row]);
    const newPieceAges = pieceAges.map(row => [...row]);
...
export const removePiecesByAge = (board, pieceAges, player, targetAge) => {
    const newBoard = board.map(row => [...row]);
    const newPieceAges = pieceAges.map(row => [...row]);
...
export const initializePieceAges = () => {
    return Array(10).fill(null).map(() => Array(10).fill(0));
};
// Count total pieces on the board (excluding blocked cells)
export const countPiecesOnBoard = (board) => {
    let count = 0;
    for (let row = 0; row < 10; row++) {
        for (let col = 0; col < 10; col++) {
            if (board[row][col] === 1 || board[row][col] === 2) {
...
export const enforcePieceCapacity = (board, pieceAges, maxCapacity = 35) => {
...
export const rearrangeBoard = (board, pieceAges) => {
...
export const swapOpponentPiecePairs = (board, pieceAges) => {
    const newBoard = board.map(row => [...row]);
    const newPieceAges = pieceAges.map(row => [...row]);
...
export const swapThreeOpponentPiecePairs = (board, pieceAges) => {
    const newBoard = board.map(row => [...row]);
    const newPieceAges = pieceAges.map(row => [...row]);
    // Collect all pieces (player 1 and player 2) with their positions
    const player1Pieces = [];
...
export const generateMudZones = (gameNumber) => {
...
export const isInMudZone = (row, col, mudZones) => {
...
export const processMudZoneEffects = (stuckPieces) => {
    const newStuckPieces = {};
...
export const generateBlockedCells = (gameNumber, currentMatch = 1) => {
...
export const createBoardWithBlocks = (gameNumber, isBlindPlay = false, currentMatch = 1) => {
