...
//
// Difficulty map summary
// ─────────────────────────────────────────────────────────────────────────────
//  Levels 1–200
//    HARD   : ending in 4, 7, or 9 (levels 10+)
//    MEDIUM : ending in 0, 3, 5, 6, or 8
//    EASY   : everything else (ending in 1 or 2)
//
//  Levels 201–600
//    HARD   : ending in 0, 2, 5, or 9
//    MEDIUM : everything else
//
//  Levels 601–2000
//    MEDIUM : ending in 1–4
//    EASY   : ending in 9
//    HARD   : everything else (endings 0, 5, 6, 7, 8)
//
//  Levels 2001+
//    HARD   : all levels
// ─────────────────────────────────────────────────────────────────────────────
String getAIDifficulty(int gameNumber) {
  ...
  // ── Levels 1–200 ──────────────────────────────────────────────────────────
  if (gameNumber <= 200) {
    // HARD: endings 4, 7, and 9 (9 only from level 10+)
    if (lastDigit == 4 || lastDigit == 7) return 'hard';
    if (lastDigit == 9 && gameNumber >= 10) return 'hard';
...
GameRules getGameRules(int gameNumber, [int? currentMatch]) {
  ...
  // Disappearing pieces
  final hasDisappearingPieces =
      (gameNumber >= 500 && gameNumber <= 1000 && gameNumber % 7 == 0) ||
  ...
  // Build description
  final rules = <String>[];
  if (mudZones) rules.add('Mud Zones');
  if (blindPlay) rules.add('Blind Play');
  if (blockedCells) {
  ...
  if (hasDisappearingPieces) rules.add('Disappearing Pieces');
  if (hasPieceCapacity) rules.add('Piece Capacity');
  if (isMatch) rules.add(matchType == 'best-of-5' ? 'Best of 5' : 'Best of 3');
