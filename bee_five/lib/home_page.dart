...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  // highestUnlockedGame: the furthest level ever reached. This is the value we
  // persist and restore on every app launch. Levels <= this are unlocked.
  //
  // currentGame: the level the bee sits on and the button shows. It equals
  // highestUnlockedGame on startup and after returning from a game. It changes
  // temporarily when the user taps a different unlocked level to replay it.
  int currentGame = 1;
  ...
  Future<void> _loadProgressOnStartup() async {
    ...
    // Step 1: apply the local pref immediately so the map and button show the
    // correct level before the network sync completes. This prevents a brief
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    // Save on inactive (iOS pre-background), paused, and detached so we catch
    // every path the OS can take when killing the app.
    ...
    if (gameMode == GameMode.adventureGame) {
      return AdventureGame(
        onBackToMenu: (int levelFromGame) {
          // Advance highestUnlockedGame if this session made progress.
          final newHighest = math.max(highestUnlockedGame, levelFromGame);

          // Persist FIRST before setState so a crash cannot lose the progress.
          _saveHighest(newHighest);

          setState(() {
            gameMode = GameMode.menu;
            highestUnlockedGame = newHighest;
            currentGame = newHighest;
          });
